...
public class HorochanChanPerformer extends ChanPerformer
{
		...
		if (jsonObject != null)
		{
			...
			try
			{
				...
				if (data.cachedPosts != null)
				{
					...
					if (posts != null)
					{
						for (Post post : posts) postNumbers.add(post.getPostNumber());
					}
				}
				if (partial)
				{
	...
	public SendPostResult onSendPost(SendPostData data) throws HttpException, ApiException, InvalidResponseException
	{
		...
		JSONObject jsonObject = new HttpRequest(uri, data.holder, data).setPostMethod(entity).setSuccessOnly(false)
				.read().getJsonObject();
		if (jsonObject == null)
		{
		...
		if (data.holder.getResponseCode() == HttpURLConnection.HTTP_OK) return null;
		JSONArray messages = jsonObject.optJSONArray("message");
		String message = CommonUtils.optJsonString(jsonObject, "message");
		if (message == null) throw new InvalidResponseException();
		if (messages != null)
		{
			try
			{
		...
		int errorType = 0;
		if (message.contains("Invalid captcha")) errorType = ApiException.SEND_ERROR_CAPTCHA;
		else if (message.contains("'Subject'")) errorType = ApiException.SEND_ERROR_EMPTY_SUBJECT;
		else if (message.contains("'Message'")) errorType = ApiException.SEND_ERROR_EMPTY_COMMENT;
			...
			InvalidResponseException
	{
		...
		JSONObject jsonObject = new HttpRequest(uri, data.holder, data).setDeleteMethod(new UrlEncodedEntity("password",
		...
		if (message == null) throw new InvalidResponseException();
		int errorType = 0;
		if (message.contains("Post not found or deleted")) errorType = ApiException.DELETE_ERROR_NO_ACCESS;
		else if (message.contains("Incorrect password")) errorType = ApiException.DELETE_ERROR_PASSWORD;
		else if (message.contains("expired")) errorType = ApiException.DELETE_ERROR_TOO_OLD;
		if (errorType != 0) throw new ApiException(errorType); else
		{
