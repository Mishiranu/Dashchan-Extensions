...
public class OnechancaChanPerformer extends ChanPerformer
{
			...
			InvalidResponseException
	{
		...
		try
		{
			...
			if (posts == null || posts.isEmpty()) throw HttpException.createNotFoundException();
	...
	public ReadPostsCountResult onReadPostsCount(ReadPostsCountData data) throws HttpException, InvalidResponseException
	{
		...
		Uri uri = locator.createThreadUri(data.boardName, data.threadNumber);
		String responseText = new HttpRequest(uri, data.holder, data).setValidator(data.validator).read().getString();
		if (!responseText.contains("<div class=\"b-blog-entry\"")) throw new InvalidResponseException();
		int count = 0;
		int index = 0;
		while (index != -1)
		{
	...
	public ReadCaptchaResult onReadCaptcha(ReadCaptchaData data) throws HttpException, InvalidResponseException
	{
		...
		if (StringUtils.isEmpty(sessionCookie)) throw new InvalidResponseException();
	...
	public SendPostResult onSendPost(SendPostData data) throws HttpException, ApiException, InvalidResponseException
	{
		...
		if (data.boardName.startsWith("news"))
		{
			if (data.threadNumber == null)
			{
				...
				if (category != null)
				{
					...
					if (title == null) throw new ApiException(ApiException.SEND_ERROR_NO_ACCESS);
				...
				if (comment.length() > maxLength)
				{
					...
					if (index >= minLength) comment = comment.substring(0, index);
					else comment = comment.substring(0, maxLength - 1) + "…";
				}
			...
			else
			{
				...
				{
					String message = matcher.group(1);
					if (message.isEmpty()) return new SendPostResult(data.threadNumber, null);
		...
		else
		{
			...
			if (data.attachments != null) data.attachments[0].addToEntity(entity, "upload");
			...
			if (matcher.find())
			{
				try
				{
					...
					for (Iterator<String> it = jsonObject.keys(); it.hasNext();) errors.add(it.next());
					if (errors.contains("text")) throw new ApiException(ApiException.SEND_ERROR_EMPTY_COMMENT);
					else if (errors.contains("captcha")) throw new ApiException(ApiException.SEND_ERROR_CAPTCHA);
			...
			InvalidResponseException
	{
		if (!data.boardName.startsWith("news") && data.postNumbers.get(0).equals(data.threadNumber))
		{
			...
			if ("true".equals(responseText)) return new SendDeletePostsResult();
			if ("false".equals(responseText)) throw new ApiException(ApiException.DELETE_ERROR_PASSWORD);
			CommonUtils.writeLog("Onechan delete message", responseText);
			throw new InvalidResponseException();
		}
		else throw new ApiException(ApiException.DELETE_ERROR_NO_ACCESS);
	}
}