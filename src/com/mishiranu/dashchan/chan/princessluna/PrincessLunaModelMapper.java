...
public class PrincessLunaModelMapper
{
	...
	public static Post createPost(JSONObject jsonObject, ChanLocator locator, String boardName) throws JSONException
	{
		...
		{
			boolean newSoftware = Integer.parseInt(no) >= 1077;
			try
			{
				ArrayList<FileAttachment> attachments = new ArrayList<>();
				attachments.add(createFileAttachment(jsonObject, locator, boardName, newSoftware));
				...
				if (filesArray != null)
				{
					for (int i = 0; i < filesArray.length(); i++)
					{
						...
						FileAttachment attachment = createFileAttachment(fileObject, locator, boardName, newSoftware);
