...
public class UboachanChanPerformer extends ChanPerformer
{
	@Override
	public ReadThreadsResult onReadThreads(ReadThreadsData data) throws HttpException, InvalidResponseException
	{
		UboachanChanLocator locator = UboachanChanLocator.get(this);
		...
		if (jsonObject != null && data.pageNumber >= 0)
		{
			try
			{
				...
				{
					threads[i] = UboachanModelMapper.createThread(threadsArray.getJSONObject(i),
			...
			if (data.isCatalog())
			{
				try
				{
					...
					for (int i = 0; i < jsonArray.length(); i++)
					{
						...
						{
							threads.add(UboachanModelMapper.createThread(threadsArray.getJSONObject(j),
		...
		if (jsonObject != null)
		{
			try
			{
				...
				if (jsonArray.length() > 0)
				{
					...
					{
						posts[i] = UboachanModelMapper.createPost(jsonArray.getJSONObject(i),
	...
	public ReadBoardsResult onReadBoards(ReadBoardsData data) throws HttpException, InvalidResponseException
	{
		...
		try
		{
			return new ReadBoardsResult(new UboachanBoardsParser(responseText).convert());
		}
	...
	public SendPostResult onSendPost(SendPostData data) throws HttpException, ApiException, InvalidResponseException
	{
		...
		if (errorMessage != null)
		{
			...
			CommonUtils.writeLog("Uboachan send message", errorMessage);
			...
			InvalidResponseException
	{
		...
		if (errorMessage != null)
		{
			...
			if (errorType != 0) throw new ApiException(errorType);
			CommonUtils.writeLog("Uboachan delete message", errorMessage);
			throw new ApiException(errorMessage);
		}
		throw new InvalidResponseException();
	}
			...
			InvalidResponseException
	{
		...
		if (errorMessage != null)
		{
			CommonUtils.writeLog("Uboachan report message", errorMessage);
			throw new ApiException(errorMessage);
		}
		throw new InvalidResponseException();
	}
