...
public class KropyvachModelMapper
{
	public static FileAttachment createFileAttachment(JSONObject jsonObject, KropyvachChanLocator locator,
			String boardName) throws JSONException
	{
		...
		//attachment.setOriginalName(filename); Original names are broken on kropyvach
		return attachment;
	}

	public static Post createPost(JSONObject jsonObject, KropyvachChanLocator locator,
			KropyvachChanConfiguration configuration, String boardName) throws JSONException
	{
		Post post = new Post();
		...
		if (name != null)
		{
			...
			if (configuration.isDefaultName(name)) post.setDefaultName(true);
			post.setName(name);
		}
	...
	public static Posts createThread(JSONObject jsonObject, KropyvachChanLocator locator,
			KropyvachChanConfiguration configuration, String boardName, boolean fromCatalog) throws JSONException
	{
		Post[] posts;
		int postsCount = 0;
		int filesCount = 0;
		if (fromCatalog)
		{
			Post post = createPost(jsonObject, locator, configuration, boardName);
		...
		else
		{
			...
			for (int i = 0; i < posts.length; i++)
			{
				...
				posts[i] = createPost(jsonObject, locator, configuration, boardName);
				if (i == 0)
				{
					postsCount = jsonObject.getInt("replies") + 1;
					filesCount = jsonObject.getInt("omitted_images") + jsonObject.getInt("images");
