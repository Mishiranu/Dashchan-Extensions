...
public class KurisachChanPerformer extends ChanPerformer
{
	...
	public ReadCaptchaResult onReadCaptcha(ReadCaptchaData data) throws HttpException, InvalidResponseException
	{
		...
		CapcthaPolicy policy = mReadCaptchaPolicies.get(data.boardName);
		try
		{
			...
			mReadCaptchaPolicies.put(data.boardName, policy);
		}
		catch (HttpException e)
		{
			int responseCode = e.getResponseCode();
			if (responseCode == HttpURLConnection.HTTP_NOT_MODIFIED) needCaptcha = policy.needCaptcha; else throw e;
		}
		if (!needCaptcha) return new ReadCaptchaResult(CaptchaState.SKIP, null);
		Uri uri = locator.buildPath("captcha.php");
		String lang;
		if (KurisachChanConfiguration.CAPTCHA_TYPE_INCH_LATIN.equals(data.captchaType)) lang = "en";
		else if (KurisachChanConfiguration.CAPTCHA_TYPE_INCH_CYRILLIC.equals(data.captchaType)) lang = "ru";
		else if (KurisachChanConfiguration.CAPTCHA_TYPE_INCH_NUMERIC.equals(data.captchaType)) lang = "num";
