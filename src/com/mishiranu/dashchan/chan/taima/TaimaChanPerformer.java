...
public class TaimaChanPerformer extends ChanPerformer {
	...
	public ReadThreadsResult onReadThreads(ReadThreadsData data) throws HttpException, InvalidResponseException {
		...
		try {
			response = new HttpRequest(uri, data).setValidator(data.validator).read();
	...
	private void checkResponse(String responseText) throws HttpException {
		if (responseText != null) {
			if (responseText.contains("<title>404 - Not Found!</title>")) {
				throw HttpException.createNotFoundException();
			}
		}
	}
	...
	private static class DisplayOrderItem implements Comparable<DisplayOrderItem> {
		...
		public int compareTo(DisplayOrderItem another) {
			return displayOrder - another.displayOrder;
		}
	}
	...
	public ReadBoardsResult onReadBoards(ReadBoardsData data) throws HttpException, InvalidResponseException {
		...
		JSONObject categoriesObject = new HttpRequest(uri, data).read().getJsonObject();
		uri = locator.createApiUri("boards.json");
		JSONObject boardsObject = new HttpRequest(uri, data).read().getJsonObject();
	...
	public SendReportPostsResult onSendReportPosts(SendReportPostsData data) throws HttpException, ApiException,
		...
		if (!postNumber.equals(data.threadNumber)) {
			uriBuilder.appendQueryParameter("parentId", data.threadNumber);
		}
